...
__export(network_exports, {
  ...
  ethSignTypedDataV4: () => ethSignTypedDataV4,
  ...
  txPoolContentFrom: () => txPoolContentFrom,
...
import { Address as Address8, HexUInt as HexUInt6 } from "@vechain/sdk-core";
...
var getEventSubscriptionUrl = (baseURL, event, indexedValues, options) => {
  const ev = typeof event === "string" ? new ABIEvent(event) : new ABIEvent(event);
...
function buildCriteria(contract, prop, args) {
  ...
  const topics = new Map(
    eventAbi.encodeFilterTopicsNoNull(args).map((topic, index) => [index, topic])
  );
  return {
    criteria: {
...
function getEventArgsMap(abi3, eventName) {
  const event = abi3.find(
...
var Contract = class {
   ...
   */
  constructor(address, abi3, thor, signer, transactionReceipt) {
    this.abi = abi3;
    this.thor = thor;
...
var ContractFactory = class {
   ...
   * @param thor An instance of ThorClient to interact with the blockchain.
   */
  constructor(abi3, bytecode, signer, thor) {
    this.abi = abi3;
  ...
  async startDeployment(deployParams, options) {
    const deployContractClause = Clause2.deployContract(
...
var ContractsModule = class {
   ...
   */
  createContractFactory(abi3, bytecode, signer) {
    return new ContractFactory(abi3, bytecode, signer, this.thor);
  }
  /**
   ...
   */
  load(address, abi3, signer) {
    return new Contract(address, abi3, this.thor, signer);
  }
  /**
  ...
  async executeMultipleClausesCall(clauses, options) {
    ...
    return response.map(
      (res, index) => clauses[index].functionAbi.decodeOutputAsArray(Hex.of(res.data))
    );
  }
  /**
  ...
  async executeTransaction(signer, contractAddress, functionAbi, functionData, options) {
    const id = await signer.sendTransaction({
      clauses: [
        ...
        Clause3.callFunction(
          Address4.of(contractAddress),
          functionAbi,
          functionData,
          VET3.of(options?.value ?? 0, Units3.wei)
        )
      ],
      gas: options?.gas,
  ...
  async getBaseGasPrice() {
    return await this.executeCall(
      ...
      ABIContract2.ofAbi(BUILT_IN_CONTRACTS.PARAMS_ABI).getFunction("get"),
...
import { Address as Address5, Hex as Hex2, ThorId as ThorId2 } from "@vechain/sdk-core";
...
var DebugModule = class {
  ...
  async traceContractCall(input, name) {
    if (input.contractInput?.to !== void 0 && input.contractInput.to !== null && !Address5.isValid(input.contractInput.to)) {
    ...
    if (input.contractInput?.data !== void 0 && !Hex2.isValid(input.contractInput.data))
      throw new InvalidDataType6(
        "DebugModule.traceContractCall()",
        `Invalid data '${input.contractInput?.data}' given as input for traceContractCall.`,
        { data: input.contractInput?.data }
      );
    if (input.contractInput?.value !== void 0 && !Hex2.isValid0x(input.contractInput.value)) {
      throw new InvalidDataType6(
        "DebugModule.traceContractCall()",
...
import { Transaction as Transaction2 } from "@vechain/sdk-core";
var GasModule = class {
  /**
   * Initializes a new instance of the `Thor` class.
   * @param thor - The Thor instance used to interact with the VeChain blockchain API.
  ...
  async estimateGas(clauses, caller, options) {
    ...
    const intrinsicGas = Number(Transaction2.intrinsicGas(clauses).wei);
...
var LogsModule = class {
  ...
  async filterEventLogs(filterOptions) {
    ...
    if (eventAbis !== void 0) {
      ...
      eventLogs.forEach((log) => {
        ...
        if (eventAbi === void 0 || eventAbi === null) {
        ...
        log.decodedData = eventAbi.decodeEventLogAsArray({
          data: Hex3.of(log.data),
          topics: log.topics.map((topic) => Hex3.of(topic))
        });
        result.push(log);
      });
    }
  ...
  removeDuplicatedAbis(eventAbis) {
    const uniqueEventAbis = /* @__PURE__ */ new Map();
...
var TransactionsModule = class {
  ...
  async sendRawTransaction(raw) {
    ...
    try {
      Transaction3.decode(HexUInt2.of(raw.slice(2)).bytes, true);
    } catch (error) {
      throw new InvalidDataType9(
        "TransactionsModule.sendRawTransaction()",
  ...
  async sendTransaction(signedTx) {
    ...
    const rawTx = Hex4.of(signedTx.encoded).toString();
  ...
  async buildTransactionBody(clauses, gas, options) {
    ...
    return {
      ...
      nonce: options?.nonce ?? Hex4.random(8).toString(),
...
import { Hex as Hex5, HexUInt as HexUInt3 } from "@vechain/sdk-core";
...
var _getDelegationSignature = async (tx, delegatorUrl, originAddress, httpClient) => {
  const rawTx = Hex5.of(tx.encoded).toString();
  ...
  try {
    ...
    return HexUInt3.of(response.signature.slice(2)).bytes;
  } catch (error) {
...
var VeChainAbstractSigner = class {
  ...
  async populateCall(transactionToPopulate) {
    if (transactionToPopulate.from === void 0 || transactionToPopulate.from === null)
      transactionToPopulate.from = Address6.checksum(
        HexUInt4.of(await this.getAddress())
      );
    else {
      if (Address6.checksum(HexUInt4.of(transactionToPopulate.from)) !== Address6.checksum(HexUInt4.of(await this.getAddress()))) {
        throw new InvalidDataType10(
          "VeChainAbstractSigner.populateCall()",
          "From address does not match the signer address.",
          {
            signerAddress: Address6.checksum(
              HexUInt4.of(await this.getAddress())
            ),
            fromAddress: Address6.checksum(
              HexUInt4.of(transactionToPopulate.from)
            )
          }
        );
      }
  ...
  _buildClauses(transaction) {
    return transaction.to !== void 0 && transaction.to !== null ? (
      ...
      [
        Clause4.deployContract(
          HexUInt4.of(transaction.data ?? 0)
        )
      ]
    );
  }
...
var VeChainPrivateKeySigner = class _VeChainPrivateKeySigner extends VeChainAbstractSigner {
   ...
   */
  async getAddress() {
    return Address7.checksum(
      HexUInt5.of(
        await Promise.resolve(
          Address7.ofPrivateKey(this.privateKey).toString()
        )
      )
    );
  }
  ...
  async signTypedData(domain, types, value) {
    return await new Promise((resolve, reject) => {
      try {
        const hash = Hex7.of(
          vechain_sdk_core_ethers2.TypedDataEncoder.hash(
            domain,
            types,
            value
          )
  ...
  async _signFlow(transaction, delegator, thorClient) {
    ...
    return delegator !== null ? await this._signWithDelegator(
      ...
      Transaction4.of(populatedTransaction).sign(this.privateKey).encoded
    ).toString();
  }
  /**
  ...
  async _signWithDelegator(unsignedTransactionBody, originPrivateKey, thorClient, delegatorOptions) {
    const originAddress = Address7.ofPrivateKey(originPrivateKey).toString();
    const unsignedTx = Transaction4.of(unsignedTransactionBody);
    if (delegatorOptions?.delegatorPrivateKey !== void 0)
      return Hex7.of(
        Transaction4.of(unsignedTransactionBody).signWithDelegator(
          originPrivateKey,
          HexUInt5.of(delegatorOptions?.delegatorPrivateKey).bytes
        ).encoded
      ).toString();
    ...
    return Hex7.of(
      Transaction4.of(unsignedTx.body, signature).encoded
    ).toString();
  }
};
...
var AbstractProviderInternalWallet = class {
  ...
  getAccountSync(addressOrIndex) {
    ...
    if (!Address8.isValid(addressOrIndex)) {
    ...
    const account = this.accounts.find(
      (account2) => Address8.checksum(HexUInt6.of(account2.address)) === Address8.checksum(HexUInt6.of(addressOrIndex))
    );
    return account ?? null;
  }
  /**
...
import { Address as Address9, HDKey, Secp256k1 as Secp256k12 } from "@vechain/sdk-core";
    ...
    super(
      [...Array(count).keys()].map((path) => {
        ...
        return {
          ...
          publicKey: Secp256k12.derivePublicKey(privateKeyBuffer),
          address: Address9.ofPrivateKey(privateKeyBuffer).toString()
        };
      }),
      options
    );
...
import { vechain_sdk_core_ethers as vechain_sdk_core_ethers3 } from "@vechain/sdk-core";
import { stringifyData } from "@vechain/sdk-errors";
var JSONRPCEthersProvider = class extends vechain_sdk_core_ethers3.JsonRpcApiProvider {
  /**
...
import {
  JSONRPCInvalidParams as JSONRPCInvalidParams36,
...
var RPC_METHODS = /* @__PURE__ */ ((RPC_METHODS2) => {
  ...
  RPC_METHODS2["txpool_inspect"] = "txpool_inspect";
  RPC_METHODS2["txpool_contentFrom"] = "txpool_contentFrom";
  RPC_METHODS2["txpool_content"] = "txpool_content";
  RPC_METHODS2["txpool_status"] = "txpool_status";
  RPC_METHODS2["eth_signTransaction"] = "eth_signTransaction";
  RPC_METHODS2["debug_traceBlockByHash"] = "debug_traceBlockByHash";
  RPC_METHODS2["debug_traceBlockByNumber"] = "debug_traceBlockByNumber";
  ...
  RPC_METHODS2["eth_signTypedData_v4"] = "eth_signTypedData_v4";
...
var getCorrectBlockNumberRPCToVeChain = (block) => {
  if (typeof block === "string") {
    if (block === "latest" || block === "finalized" || block === "safe" || block === "pending")
      return "best";
    if (block === "earliest") return Hex8.of(0).toString();
    return block;
  }
  if (block.blockNumber !== void 0) {
    return Hex8.of(block.blockNumber).toString();
  }
...
import { HexUInt as HexUInt8, Quantity as Quantity2, ZERO_BYTES as ZERO_BYTES2 } from "@vechain/sdk-core";

// src/provider/utils/formatter/transactions/formatter.ts
import { Hex as Hex9, HexUInt as HexUInt7, Quantity, ZERO_BYTES } from "@vechain/sdk-core";
...
var _formatTransactionToRPC = (tx, blockHash, blockNumber, chainId, txIndex) => {
  return {
    ...
    from: tx.origin,
    gas: Quantity.of(HexUInt7.of(tx.gas).bi).toString(),
    chainId,
    hash: tx.id,
    nonce: tx.nonce,
    ...
    value: tx.clauses[0]?.value !== void 0 ? Quantity.of(HexUInt7.of(tx.clauses[0].value).bi).toString() : "",
    // Unsupported fields
    gasPrice: "0x0",
    type: "0x0",
    v: "0x0",
...
function formatTransactionReceiptToRPCStandard(transactionHash, receipt, transaction, blockContainsTransaction, chainId) {
  ...
  return {
    ...
    transactionIndex: Quantity.of(transactionIndex).toString(),
    // Incompatible fields
    logsBloom: Hex9.of(ZERO_BYTES(256)).toString(),
...
var formatToRPCStandard2 = (block, chainId) => {
  ...
  return {
    ...
    totalDifficulty: "0x0",
    uncles: [],
    sha3Uncles: HexUInt8.of(ZERO_BYTES2(32)).toString(),
    nonce: HexUInt8.of(ZERO_BYTES2(8)).toString(),
    logsBloom: HexUInt8.of(ZERO_BYTES2(256)).toString(),
    extraData: "0x",
    baseFeePerGas: "0x0",
    mixHash: HexUInt8.of(ZERO_BYTES2(32)).toString()
  };
};
...
import { JSONRPCInternalError, stringifyData as stringifyData2 } from "@vechain/sdk-errors";
...
import {
  ...
  JSONRPCInvalidParams as JSONRPCInvalidParams11,
...
var RPCMethodsMap = (thorClient, provider) => {
  return {
    ...
    ["txpool_inspect" /* txpool_inspect */]: async () => {
      return await txPoolInspect();
    },
    ["txpool_content" /* txpool_content */]: async () => {
      return await txPoolContent();
    },
    ["txpool_contentFrom" /* txpool_contentFrom */]: async (params) => {
      return await txPoolContentFrom(params);
    },
    ["txpool_status" /* txpool_status */]: async () => {
      return await txPoolStatus();
    },
    ["debug_traceBlockByHash" /* debug_traceBlockByHash */]: async (params) => {
      return await debugTraceBlockByHash(thorClient, params);
    },
    ["debug_traceBlockByNumber" /* debug_traceBlockByNumber */]: async (params) => {
      return await debugTraceBlockByNumber(thorClient, params);
    },
    ["eth_signTypedData_v4" /* eth_signTypedData_v4 */]: async (params) => {
...
import {
  ...
  stringifyData as stringifyData12
} from "@vechain/sdk-errors";
var ethGetBlockByHash = async (thorClient, params) => {
  ...
  try {
    ...
    throw new JSONRPCInternalError11(
      ...
      {
        params: stringifyData12(params),
        url: thorClient.httpClient.baseURL,
        innerError: stringifyData12(e)
      }
    );
  }
};
...
import { JSONRPCInvalidParams as JSONRPCInvalidParams14 } from "@vechain/sdk-errors";
import { Hex as Hex11, Address as Address10, Secp256k1 as Secp256k13 } from "@vechain/sdk-core";
var ethGetTransactionCount = async (params) => {
  ...
  return Hex11.of(Secp256k13.randomBytes(6)).toString();
};
...
import {
  ...
  JSONRPCInvalidParams as JSONRPCInvalidParams15,
...
import { JSONRPCInternalError as JSONRPCInternalError15, stringifyData as stringifyData16 } from "@vechain/sdk-errors";
...
import { JSONRPCInvalidParams as JSONRPCInvalidParams17 } from "@vechain/sdk-errors";
import { Hex as Hex12, Keccak256 as Keccak2564 } from "@vechain/sdk-core";
var web3Sha3 = async (params) => {
  if (params.length !== 1 || typeof params[0] !== "string" || !Hex12.isValid(params[0]))
...
import { Hex as Hex13 } from "@vechain/sdk-core";
import {
...
var ethSubscribe = async (thorClient, params, provider) => {
  ...
  const subscriptionId = Hex13.random(16).digits;
...
import { JSONRPCInternalError as JSONRPCInternalError17, stringifyData as stringifyData18 } from "@vechain/sdk-errors";
...
import {
  ...
  stringifyData as stringifyData20
} from "@vechain/sdk-errors";
...
import { JSONRPCInternalError as JSONRPCInternalError20, stringifyData as stringifyData21 } from "@vechain/sdk-errors";
...
import { JSONRPCInvalidParams as JSONRPCInvalidParams21 } from "@vechain/sdk-errors";
...
import {
  ...
  JSONRPCInvalidParams as JSONRPCInvalidParams23,
...
import {
  ...
  stringifyData as stringifyData23
} from "@vechain/sdk-errors";
...
import { JSONRPCInvalidParams as JSONRPCInvalidParams25 } from "@vechain/sdk-errors";
import { ThorId as ThorId9 } from "@vechain/sdk-core";
var ethGetUncleByBlockHashAndIndex = async (params) => {
  if (params.length !== 2 || typeof params[0] !== "string" || !ThorId9.isValid(params[0]) || typeof params[1] !== "string")
...
import { JSONRPCInvalidParams as JSONRPCInvalidParams26 } from "@vechain/sdk-errors";
var ethGetUncleByBlockNumberAndIndex = async (params) => {
  if (params.length !== 2 || typeof params[0] !== "string" || typeof params[1] !== "string")
...
import { ThorId as ThorId10 } from "@vechain/sdk-core";
import { JSONRPCInvalidParams as JSONRPCInvalidParams27 } from "@vechain/sdk-errors";
var ethGetUncleCountByBlockHash = async (params) => {
  if (params.length !== 1 || typeof params[0] !== "string" || !ThorId10.isValid(params[0]))
...
import { JSONRPCInvalidParams as JSONRPCInvalidParams28 } from "@vechain/sdk-errors";
var ethGetUncleCountByBlockNumber = async (params) => {
  if (params.length !== 1 || typeof params[0] !== "string")
    throw new JSONRPCInvalidParams28(
...
import { JSONRPCInvalidParams as JSONRPCInvalidParams29, stringifyData as stringifyData24 } from "@vechain/sdk-errors";
var ethRequestAccounts = async (provider) => {
  const accounts = await ethAccounts(provider);
  if (accounts.length === 0)
    throw new JSONRPCInvalidParams29(
      "eth_getTransactionReceipt()",
      -32602,
      'Method "ethRequestAccounts" failed.',
      {
        provider: stringifyData24(provider)
      }
    );
  return accounts;
};
...
// src/provider/utils/rpc-mapper/methods-map/methods/eth_signTypedData_v4/eth_signTypedData_v4.ts
import {
  JSONRPCInternalError as JSONRPCInternalError23,
  JSONRPCInvalidParams as JSONRPCInvalidParams30,
  stringifyData as stringifyData25
} from "@vechain/sdk-errors";
import { Address as Address11 } from "@vechain/sdk-core";
var ethSignTypedDataV4 = async (thorClient, params, provider) => {
  if (params.length !== 2 || typeof params[0] !== "string" || !Address11.isValid(params[0]) || typeof params[1] !== "object")
    throw new JSONRPCInvalidParams30(
      "eth_signTypedDataV4",
      -32602,
  ...
  if (provider?.wallet === void 0 || await provider.getSigner(params[0]) === null) {
    throw new JSONRPCInvalidParams30(
      "eth_signTypedDataV4",
      -32602,
...
import { JSONRPCInternalError as JSONRPCInternalError24, stringifyData as stringifyData26 } from "@vechain/sdk-errors";
...
import { JSONRPCInternalError as JSONRPCInternalError25, stringifyData as stringifyData27 } from "@vechain/sdk-errors";
...
// src/provider/utils/rpc-mapper/methods-map/methods/txpool_content/txpool_content.ts
var txPoolContent = async () => {
  return await Promise.resolve({});
};

// src/provider/utils/rpc-mapper/methods-map/methods/txpool_contentFrom/txpool_contentFrom.ts
import { JSONRPCInvalidParams as JSONRPCInvalidParams31 } from "@vechain/sdk-errors";
import { addressUtils } from "@vechain/sdk-core";
var txPoolContentFrom = async (params) => {
  if (params.length !== 1 || typeof params[0] !== "string" || !addressUtils.isAddress(params[0]))
    throw new JSONRPCInvalidParams31(
      "eth_getBalance",
      -32602,
...
// src/provider/utils/rpc-mapper/methods-map/methods/txpool_inspect/txpool_inspect.ts
var txPoolInspect = async () => {
  return await Promise.resolve({});
};

// src/provider/utils/rpc-mapper/methods-map/methods/txpool_status/txpool_status.ts
var txPoolStatus = async () => {
  return await Promise.resolve({});
};
...
// src/provider/utils/rpc-mapper/methods-map/methods/debug_traceBlockByHash/debug_traceBlockByHash.ts
import { ThorId as ThorId11 } from "@vechain/sdk-core";
import {
  JSONRPCInternalError as JSONRPCInternalError26,
  JSONRPCInvalidParams as JSONRPCInvalidParams32,
  stringifyData as stringifyData28
} from "@vechain/sdk-errors";
var debugTraceBlockByHash = async (thorClient, params) => {
  if (params.length !== 2 || typeof params[0] !== "string" || !ThorId11.isValid(params[0]) || typeof params[1] !== "object")
    throw new JSONRPCInvalidParams32(
  ...
  try {
    ...
    throw new JSONRPCInternalError26(
      "debug_traceBlockByHash()",
      -32603,
...
// src/provider/utils/rpc-mapper/methods-map/methods/debug_traceBlockByNumber/debug_traceBlockByNumber.ts
import {
  JSONRPCInternalError as JSONRPCInternalError27,
  JSONRPCInvalidParams as JSONRPCInvalidParams33,
  stringifyData as stringifyData29
} from "@vechain/sdk-errors";
var debugTraceBlockByNumber = async (thorClient, params) => {
  if (params.length !== 2 || typeof params[0] !== "string" || typeof params[1] !== "object")
    throw new JSONRPCInvalidParams33(
  ...
  try {
    ...
    throw new JSONRPCInternalError27(
      "debug_traceBlockByNumber()",
      -32603,
      'Method "debug_traceBlockByNumber" failed.',
      {
        params: stringifyData29(params),
        url: thorClient.httpClient.baseURL,
        innerError: stringifyData29(e)
      }
    );
  }
};
...
import {
  JSONRPCInternalError as JSONRPCInternalError28,
  JSONRPCInvalidParams as JSONRPCInvalidParams34,
  stringifyData as stringifyData30
} from "@vechain/sdk-errors";
var ethGetBlockReceipts = async (thorClient, params) => {
  if (params.length !== 1 || typeof params[0] !== "string")
    throw new JSONRPCInvalidParams34(
  ...
  try {
    ...
    throw new JSONRPCInternalError28(
      "ethGetBlockReceipts()",
      -32603,
      'Method "ethGetBlockReceipts" failed.',
      {
        params: stringifyData30(params),
        url: thorClient.httpClient.baseURL,
        innerError: stringifyData30(e)
      }
    );
  }
};
...
import {
  JSONRPCInternalError as JSONRPCInternalError29,
  JSONRPCInvalidParams as JSONRPCInvalidParams35,
  stringifyData as stringifyData31
} from "@vechain/sdk-errors";
var ethSignTransaction = async (thorClient, params, provider) => {
  if (params.length !== 1 || typeof params[0] !== "object")
    throw new JSONRPCInvalidParams35(
      "eth_signTransaction",
      -32602,
  ...
  try {
    ...
    throw new JSONRPCInternalError29(
      "eth_signTransaction()",
      -32603,
      'Method "eth_signTransaction" failed.',
      {
        params: stringifyData31(params),
        url: thorClient.httpClient.baseURL
      },
      error
    );
...
var VeChainProvider = class extends EventEmitter2 {
  ...
  constructor(thorClient, wallet, enableDelegation = false) {
    ...
    if (enableDelegation && wallet?.delegator === void 0) {
      throw new JSONRPCInvalidParams36(
...
import {
  JSONRPCInternalError as JSONRPCInternalError30,
  stringifyData as stringifyData32,
  VechainSDKError
} from "@vechain/sdk-errors";
  ...
  async request(args) {
    try {
      if (this.debug) {
        ...
        VeChainSDKLogger37("log").log({
          title: `Sending request - ${args.method}`,
          messages: [
            `params: ${stringifyData32(args.params)}`,
            `accounts: ${stringifyData32(accounts)}`,
            `delegator: ${stringifyData32(delegator)}`,
      ...
      if (this.debug) {
        VeChainSDKLogger37("log").log({
          ...
          messages: [`result: ${stringifyData32(result)}`]
        });
      }
      return result;
    } catch (error) {
      if (this.debug) {
        VeChainSDKLogger37("error").log(
          new JSONRPCInternalError30(
            args.method,
            -32603,
