...
var Hex = class _Hex extends String {
  /**
  ...
  constructor(sign4, digits, normalize = (digits2) => digits2.toLowerCase()) {
    ...
    super((sign4 < 0 ? "-0x" : "0x") + normalizedDigits);
   ...
   */
  get abs() {
    return new _Hex(_Hex.POSITIVE, this.hex);
  }
  /**
   ...
   */
  get bi() {
    return BigInt(this.sign) * hexToNumber(this.hex);
  }
  /**
   ...
   */
  get bytes() {
    return hexToBytes(this.alignToBytes().hex);
  }
  /**
  ...
  get n() {
    ...
    throw new import_sdk_errors4.InvalidCastType(
      "Hex.n",
   ...
   */
  alignToBytes() {
    return this.hex.length % 2 === 0 ? this : new _Hex(this.sign, "0" + this.hex);
  }
  /**
  ...
  fit(digits) {
    if (digits < this.hex.length) {
      ...
      if (this.hex.length - cue === digits) {
        return new _Hex(this.sign, this.hex.slice(cue));
      }
  ...
  static isValid0x(exp) {
    return _Hex.REGEX_PREFIX.test(exp) && _Hex.isValid(exp);
  }
  /**
  ...
  static of(exp) {
    try {
      ...
      throw new import_sdk_errors4.InvalidDataType("Hex.of", "not an hexadecimal string", {
        exp
      });
    } catch (e) {
      throw new import_sdk_errors4.InvalidDataType(
   ...
   */
  static random(bytes3) {
    if (bytes3 > 0) {
      return _Hex.of(randomBytes(bytes3));
    }
    throw new import_sdk_errors4.InvalidDataType("Hex.random", "bytes argument not > 0", {
      bytes: bytes3
    });
  }
};
...
var HexInt = class _HexInt extends Hex {
  ...
  get n() {
    ...
    throw new import_sdk_errors5.InvalidDataType("HexInt.of", "not in the safe number range", {
      bi: `${bi}`,
      hex: this.toString()
    });
  }
  ...
  static of(exp) {
    try {
      if (exp instanceof Hex) {
        return new _HexInt(exp);
      }
      if (typeof exp === "number") {
        if (Number.isInteger(exp)) {
          return new _HexInt(Hex.of(BigInt(exp)));
        }
        throw new import_sdk_errors5.InvalidDataType("HexInt.of", "not an integer", {
          exp
        });
      }
      return new _HexInt(Hex.of(exp));
    } catch (e) {
...
var HexUInt = class _HexUInt extends HexInt {
  ...
  constructor(hi) {
    if (hi.sign >= Hex.POSITIVE) {
      ...
      throw new import_sdk_errors6.InvalidDataType("HexUInt.constructor", "not positive", {
        hi
      });
    }
  }
  ...
  static of(exp) {
    try {
      return new _HexUInt(HexInt.of(exp));
    } catch (e) {
...
var ThorId = class _ThorId extends Hex {
  /**
   ...
   */
  static isValid(exp) {
    return Hex.isValid(exp) && Hex.REGEX_PREFIX.test(exp) ? exp.length === _ThorId.DIGITS + 2 : exp.length === _ThorId.DIGITS;
  }
  /**
   ...
   */
  static isValid0x(exp) {
    return Hex.REGEX_PREFIX.test(exp) && _ThorId.isValid(exp);
  }
  /**
  ...
  static of(exp) {
    ...
    return new _ThorId(Hex.of(exp).fit(_ThorId.DIGITS));
  }
};
...
var Txt = class _Txt extends String {
  ...
  get bi() {
    try {
      ...
      throw new import_sdk_errors8.InvalidCastType(
        "Txt.bi()",
...
function blake2b256(data, returnType = "buffer") {
  ...
  if (data instanceof Uint8Array) {
    ...
    return returnType === "hex" ? Hex.of(hash4).toString() : hash4;
  } else {
...
function blake2b256OfHex(hex, returnType = "buffer") {
  ...
  try {
    const hash4 = blake2b256OfArray(Hex.of(hex).bytes);
    return returnType === "hex" ? Hex.of(hash4).toString() : hash4;
  } catch (e) {
    throw new import_sdk_errors9.InvalidDataType(
      "blake2b256OfHex",
...
function sha2562(data, returnType = "buffer") {
  ...
  return returnType === "buffer" ? hash4 : Hex.of(hash4).toString();
}
...
var _packData = (obj, profile, context) => {
  ...
  if (!Array.isArray(obj)) {
    throw new import_sdk_errors12.InvalidRLP(
      "_packData()",
...
var BufferKind = class extends ScalarKind {
  ...
  data(data, context) {
    if (!Buffer.isBuffer(data))
      throw new import_sdk_errors13.InvalidRLP(
        "BufferKind.data()",
...
var validateNumericKindData = (data, context) => {
  if (typeof data !== "number" && typeof data !== "string") {
    throw new import_sdk_errors14.InvalidRLP(
...
var assertValidHexBlobKindData = (data, context) => {
  if (typeof data !== "string") {
    throw new import_sdk_errors15.InvalidRLP(
...
var assertFixedHexBlobKindData = (data, context, bytes3) => {
  if (data.length !== bytes3 * 2 + 2) {
    throw new import_sdk_errors16.InvalidRLP(
      "assertFixedHexBlobKindData()",
...
var assertCompactFixedHexBlobBuffer = (buffer, context, bytes3) => {
  if (buffer.length > bytes3) {
    throw new import_sdk_errors17.InvalidRLP(
...
var decodeBufferToHexWithLeadingZeros = (buffer, bytes3) => {
  return Hex.of(buffer).fit(bytes3 * 2).toString();
};
...
var isInBloom = (filter, k, data) => {
  if (!isBloom(filter)) {
    throw new import_sdk_errors19.InvalidBloom(
  ...
  if (!Number.isInteger(k) || k <= 0) {
    throw new import_sdk_errors19.InvalidBloomParams(
      "bloomUtils.isInBloom()",
...
var isAddressInBloom = (filter, k, address) => {
  if (!addressUtils.isAddress(address)) {
    throw new import_sdk_errors19.InvalidAddress(
...
function bigNumberOf(value) {
  ...
  if (bn.isNaN())
    throw new import_sdk_errors22.InvalidDataType("unitUtils.bigNumberOf()", "Not a number.", {
...
var isRevisionAccount = (revision) => {
  return revision === "best" || revision === "finalized" || typeof revision === "string" && Hex.isValid0x(revision) || typeof revision === "string" && dataUtils.isDecimalString(revision) || typeof revision === "number" && revision >= 0;
};
...
function inflatePublicKey(publicKey) {
  ...
  if (prefix !== 4) {
    ...
    const p = secp256k1.ProjectivePoint.fromAffine(
      secp256k1.ProjectivePoint.fromHex(Hex.of(x).hex).toAffine()
    );
...
function recover(messageHash, sig) {
  if (!isValidMessageHash(messageHash)) {
    throw new import_sdk_errors23.InvalidSecp256k1MessageHash(
      "secp256k1.sign()",
  ...
  if (sig.length !== SIGNATURE_LENGTH)
    throw new import_sdk_errors23.InvalidSecp256k1Signature(
      "secp256k1.recover()",
...
function toERC55Checksum(address) {
  if (!isAddress(address)) {
    throw new import_sdk_errors24.InvalidAddress(
  ...
  const hash4 = Hex.of(keccak256(digits)).hex;
  let result = "0x";
...
function verify(cert) {
  if (cert.signature === void 0 || cert.signature === null) {
    throw new import_sdk_errors25.CertificateSignature(
      "certificate.verify()",
...
function fromMnemonic(words, path = VET_DERIVATION_PATH) {
  ...
  try {
    ...
    throw new import_sdk_errors27.InvalidHDNodeMnemonic(
      "HDNode.fromMnemonic()",
  ...
  try {
    ...
    throw new import_sdk_errors27.InvalidHDNode(
      "HDNode.fromMnemonic()",
...
function fromPrivateKey2(privateKey, chainCode) {
  if (privateKey.length === 32) {
    ...
    try {
      ...
      throw new import_sdk_errors27.InvalidSecp256k1PrivateKey(
...
async function decrypt(keystore4, password) {
  if (!isValid(keystore4)) {
    throw new import_sdk_errors28.InvalidKeystore(
      "keystore.decrypt()",
  ...
  try {
    ...
    throw new import_sdk_errors28.InvalidKeystoreParams(
      "keystore.decrypt()",
...
function decodeScryptParams(keystore4) {
  ...
  if (N <= 0 || (N & N - 1) !== 0)
    throw new import_sdk_errors29.InvalidKeystoreParams(
...
function decryptKeystore(keystore4, password) {
  try {
    ...
    if (keystore4.address !== "" && address !== addressUtils.toERC55Checksum(
      Hex.of(keystore4.address).toString()
    )) {
      throw new import_sdk_errors29.InvalidKeystoreParams(
        "(EXPERIMENTAL) keystore.decryptKeystore()",
...
function deriveAddress(words, path = "m/0") {
  ...
  try {
    ...
    throw new import_sdk_errors30.InvalidHDNode(
...
function generate(wordlistSize, randomGenerator) {
  if (wordlistSize !== void 0 && !MNEMONIC_WORDLIST_ALLOWED_SIZES.includes(wordlistSize)) {
    throw new import_sdk_errors30.InvalidHDNodeMnemonic(
      "mnemonic.generate()",
...
var Transaction = class _Transaction {
  ...
  constructor(body, signature) {
    if (!_Transaction.isValidBody(body)) {
      throw new import_sdk_errors31.InvalidTransactionField(
    ...
    if (signature !== void 0 && !this._isSignatureValid(signature)) {
      throw new import_sdk_errors31.InvalidSecp256k1Signature(
        "Transaction constructor",
  ...
  get delegator() {
    if (!this.isDelegated)
      throw new import_sdk_errors31.NotDelegatedTransaction(
        "Transaction.delegator()",
        "Transaction is not delegated. Delegate information is unavailable.",
        void 0
      );
    if (!this.isSigned)
      throw new import_sdk_errors31.UnavailableTransactionField(
        "Transaction.delegator()",
...
function sign3(transactionBody, signerPrivateKey) {
  if (!secp256k12.isValidPrivateKey(signerPrivateKey)) {
    throw new import_sdk_errors32.InvalidSecp256k1PrivateKey(
  ...
  if (transactionToSign.isDelegated)
    throw new import_sdk_errors32.InvalidTransactionField(
      `TransactionHandler.sign()`,
...
function signWithDelegator(transactionBody, signerPrivateKey, delegatorPrivateKey) {
  ...
  const transactionToSign = new Transaction(transactionBody);
  if (!transactionToSign.isDelegated)
    throw new import_sdk_errors32.NotDelegatedTransaction(
      "signWithDelegator()",
